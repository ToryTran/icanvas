...
const dbSourceMap = new Map();
const dbScraped = new Set();
...
const DB = winston.createLogger({
  ...
  transports: [
    new winston.transports.File({
      filename: `${path.join(__dirname, `/data`)}/db_scraped.log`,
    }),
  ],
});
    ...
    for (let i = 1; i <= 2; i++) {
      ...
      selector('body')
        ...
        .each((_, element) => {
          ...
          if (url && productId && !dbSourceMap.has(productId))
            DB.info({
              productId,
              url,
            });
  ...
  await getFileDataByLine(
    ...
    (line) => {
      ...
      if (p.message.productId) {
        dbSourceMap.set(p.message.productId, p.message);
      }
    }
  );
  await getFileDataByLine(
    `${path.join(__dirname, `/data`)}/db_scraped.log`,
    (line) => {
      const p = getObjectFromLog(line);
      if (p.message.productId) {
        dbScraped.set(p.message.productId);
      }
    }
  );
  console.log('DB size: ', dbSourceMap.size);
  console.log('DB scraped size: ', dbScraped.size);
  // await doJob();
  console.log('FINISH');
})();
